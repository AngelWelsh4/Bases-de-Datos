/*
TEE C:\Users\Angel\Desktop\ep8_AngelOcampo.txt
...
# 1.
/*
SELECT sku, nombre, series, precio, color, modelo, fechaCompra AS 'fechacompra'
FROM carritosAOG
...
# 2.
/*
SELECT MONTHNAME(fechacompra) AS 'mes', COUNT(sku) AS 'Total carritos2010'
FROM v_autos3erTrim
WHERE modelo=2010
GROUP BY MONTHNAME(fechacompra)
ORDER BY MONTH(fechacompra);
*/
CREATE OR REPLACE VIEW v_reporte2 AS (SELECT MONTHNAME(fechacompra) AS 'mes', COUNT(sku) AS 'Total carritos2010'
FROM v_autos3erTrim
WHERE modelo=2010
GROUP BY MONTHNAME(fechacompra)
ORDER BY MONTH(fechacompra));
...
# 3.
/*
SELECT MONTHNAME(fechacompra) AS 'mes', COUNT(sku) AS 'Total carritos azules'
FROM v_autos3erTrim
...
# 4.
/*
SELECT fechacompra AS 'fechaCompra', COUNT(sku) AS 'Total carritos comprados por día'
FROM v_autos3erTrim
